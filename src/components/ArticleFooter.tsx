...
export default function ArticleFooter({ currentSlug }: { currentSlug: string }) {
    ...
    return (
        <div className="max-w-[1400px] mx-auto px-6 md:px-8 pb-20">
            {/* Next Step CTA Banner */}
            <div className="mb-12 mt-16">
                <Link
                    href="/geo"
                    className="block relative overflow-hidden bg-gray-900 rounded-2xl p-8 md:p-10 text-white group hover:bg-black transition-colors shadow-xl"
                >
                    ...
                    <div className="relative z-10 flex items-center justify-between gap-6">
                        <div>
                            ...
                            <p className="text-2xl md:text-3xl font-bold mb-2 group-hover:translate-x-1 transition-transform">
                                GEO Danışmanlığı: Kapsamlı Rehber
                            </p>
                            <p className="text-gray-400 max-w-xl text-base md:text-lg opacity-80">
                        ...
                        <div className="hidden sm:flex w-14 h-14 bg-white/10 rounded-full items-center justify-center group-hover:bg-violet-600 group-hover:scale-110 transition-all duration-300 shrink-0">
                    ...
                    <ul className="space-y-5 mb-10">
                        {relatedArticles.map((article, idx) => (
                            <li key={idx} className="list-none">
                                <Link href={article.href} className="inline-flex items-center text-violet-600 hover:underline font-bold group text-base md:text-lg">
                                    <span className="w-2 h-2 rounded-full bg-violet-600 mr-3 shrink-0 group-hover:scale-125 transition-all" />
                                    {article.title}
                                </Link>
            ...
            <div className="pt-2 pb-4 text-sm text-gray-400 font-medium font-sans border-t border-gray-100">
