import { NextResponse } from 'next/server';

export const dynamic = 'force-static';

export function GET() {
  const content = `# Bahattin Yaylagül – SEO & GEO Danışmanı

> Arama motorları ve yapay zeka sistemleri için içerik optimizasyonu konusunda uzmanlaşmış bağımsız danışman.

## Hakkında

Bahattin Yaylagül, SEO (Search Engine Optimization) ve GEO (Generative Engine Optimization) alanlarında uzmanlaşmış bir dijital pazarlama danışmanıdır. Markaların hem geleneksel arama motorlarında hem de yapay zeka destekli sistemlerde görünürlüğünü artırma konusunda veri odaklı stratejiler sunar.

- **Uzmanlık Alanları:** SEO Danışmanlığı, GEO (Generative Engine Optimization), Teknik SEO, İçerik Stratejisi
- **Dil:** Türkçe
- **Konum:** Türkiye
- **Web Sitesi:** https://bahattinyaylagul.com

## Ana Sayfalar

- [Ana Sayfa](https://bahattinyaylagul.com): SEO ve GEO danışmanlık hizmetleri tanıtım sayfası
- [SEO Danışmanlığı](https://bahattinyaylagul.com/seo): Arama motoru optimizasyonu hizmetleri ve yaklaşımları
- [GEO Danışmanlığı](https://bahattinyaylagul.com/geo): Generative Engine Optimization – yapay zeka için içerik optimizasyonu
- [Hakkımda](https://bahattinyaylagul.com/hakkimda): Danışman profili, deneyim ve uzmanlık alanları
- [Referanslar](https://bahattinyaylagul.com/referanslar): Müşteri referansları ve başarı hikayeleri
- [Teknik SEO](https://bahattinyaylagul.com/seo/teknik-seo): Teknik SEO rehberi ve best practice'ler
- [SEO Rehberi](https://bahattinyaylagul.com/seo-rehberi): Kapsamlı SEO kılavuzu

## SEO Konu Sayfaları

- [Arama Niyeti ve Anahtar Kelime Haritalama](https://bahattinyaylagul.com/seo/arama-niyeti): Kullanıcı arama niyetinin analizi ve anahtar kelime stratejisi
- [Semantik Yazım ve NLP](https://bahattinyaylagul.com/seo/semantik-yazim): Doğal dil işlemeye uygun semantik içerik oluşturma
- [İç Linkleme ve Silo Mimarisi](https://bahattinyaylagul.com/seo/silo-mimarisi): İç link yapısı ve konusal silo stratejisi
- [Tarama ve İndeksleme](https://bahattinyaylagul.com/seo/tarama): Crawl & Index süreçlerinin optimizasyonu
- [Indexing Ekosistemi](https://bahattinyaylagul.com/seo/indeksleme): Dizine ekleme süreci ve index yönetimi
- [Teknik SEO: Altyapı ve Performans](https://bahattinyaylagul.com/seo/teknik-seo): Site altyapısı, hız ve teknik optimizasyon
- [İçerik Optimizasyonu: Bilgi Kazancı](https://bahattinyaylagul.com/seo/icerik-optimizasyonu): İçerik kalitesi ve bilgi kazancı odaklı optimizasyon
- [Topikal Otorite](https://bahattinyaylagul.com/seo/topikal-otorite): Topical Authority oluşturma ve konu hakimiyeti
- [Sıralama ve Algoritmik Değerleme](https://bahattinyaylagul.com/seo/siralama): Google sıralama faktörleri ve algoritma analizi
- [E-E-A-T ve Güven Sinyalleri](https://bahattinyaylagul.com/seo/eeat-sinyalleri): Deneyim, Uzmanlık, Otorite ve Güvenilirlik sinyalleri
- [Bilgi Kazancı](https://bahattinyaylagul.com/seo/bilgi-kazanci): Benzersiz değer sunan içerik stratejisi
- [Site İçi SEO](https://bahattinyaylagul.com/seo/site-ici-seo): On-Page SEO optimizasyon rehberi
- [Site Dışı SEO](https://bahattinyaylagul.com/seo/site-disi-seo): Off-Page SEO ve backlink stratejileri
- [Yerel SEO](https://bahattinyaylagul.com/seo/yerel-seo): Yerel arama optimizasyonu ve Google Business Profile
- [Core Web Vitals](https://bahattinyaylagul.com/seo/core-web-vitals): LCP, FID, CLS ve sayfa deneyimi metrikleri
- [Yapısal Veri (Structured Data)](https://bahattinyaylagul.com/seo/yapisal-veri): Schema.org markup ve zengin sonuçlar
- [Tarama Bütçesi](https://bahattinyaylagul.com/seo/tarama-butcesi): Crawl budget yönetimi ve optimizasyonu
- [Log Analizi](https://bahattinyaylagul.com/seo/log-analizi): Sunucu log dosyalarının SEO analizi
- [Tarama ve İndeksleme Rehberi](https://bahattinyaylagul.com/seo/tarama-ve-indeksleme): Kapsamlı crawl ve indexing rehberi

## GEO & İçgörüler Makaleleri

- [GEO Rehberi](https://bahattinyaylagul.com/geo): Generative Engine Optimization kapsamlı rehber
- [ChatGPT GEO Rehberi](https://bahattinyaylagul.com/geo/chatgpt-geo): ChatGPT'nin yanıtlarında kaynak göstermesini sağlamanın tam rehberi
- [Perplexity GEO Rehberi](https://bahattinyaylagul.com/geo/perplexity-geo): Perplexity AI aramalarında alıntılanmak ve kaynak olarak önerilmek için yapılması gereken optimizasyon pratikleri
- [AI Overviews Nedir?](https://bahattinyaylagul.com/geo/ai-overviews-nedir): AI Overviews nasıl çalışır, SEO'ya etkisi ve içerik stratejisi
- [llms.txt Nedir?](https://bahattinyaylagul.com/geo/llms-txt-nedir): Yapay zeka modellerine yönelik llms.txt dosyasının önemi ve nasıl oluşturulacağı
- [AI Marka Mention Etkisi](https://bahattinyaylagul.com/geo/ai-marka-mention-etkisi): Yapay zeka sistemlerinde marka anılmalarının SEO'ya etkisi
- [Reddit & Forum Mention Etkisi](https://bahattinyaylagul.com/geo/reddit-forum-mention-etkisi): Forum ve sosyal platform atıflarının arama sıralamalarına etkisi

## İletişim

Danışmanlık hizmetleri için iletişime geçmek amacıyla [ana sayfa iletişim formu](https://bahattinyaylagul.com/#iletisim) kullanılabilir.

## Bağlantılar

- LinkedIn: https://www.linkedin.com/in/bahattin-yaylagul/
- Medium: https://medium.com/@bahattinyaylagl
`;

  return new NextResponse(content, {
    status: 200,
    headers: {
      'Content-Type': 'text/plain; charset=utf-8',
      'Cache-Control': 'public, max-age=86400, stale-while-revalidate=604800',
    },
  });
}
