...
const headings = [
    {
        "id": "neden-bu-sayfa",
        "text": "Bu sayfa neden diğer her şeyin \"neden\"i?"
    },
    {
        "id": "rag-nedir",
        "text": "RAG nedir, neden var?"
    },
    {
        "id": "sekiz-adim",
        "text": "Zincir adım adım: belgeden cevaba sekiz durak"
    },
    {
        "id": "grounding-fark",
        "text": "Grounding ile RAG aynı şey değil"
    },
    {
        "id": "rag-yetersiz",
        "text": "RAG'ın göremediği yerler"
    },
    {
        "id": "turkce-retrieval",
        "text": "Türkçe'de bu zincir aynı çalışmıyor"
    },
    {
        "id": "cikarim",
        "text": "Zincirden çıkan içerik kararları"
    },
    {
        "id": "sss",
...
export default function ArticleContent() {
    ...
    sizi bilmesine değil, bu zincirin hangi halkasında elendiğinize bağlı.
                ...
                <aside className="tldr mb-12 p-6 md:p-8 bg-violet-50/50 border border-violet-100/80 rounded-3xl shadow-sm">
                    ...
                    <p className="text-gray-800 font-bold text-[15px] leading-relaxed mb-4"><strong>Yapay zeka araması bir sıralama yarışı değil, bir eleme zinciri: içeriğiniz taranabilir, parçalanabilir, eşleşebilir ve alıntılanabilir olmadan cevaba giremez.</strong></p>
                    <ul className="space-y-3.5 text-gray-700 text-[15px] leading-relaxed list-none pl-0 my-0"><li className="flex items-start gap-3"><span className="text-violet-600 font-bold shrink-0 mt-0.5">✦</span><span>Model cevabı üretmeden önce dört ayrı elemeden geçiyorsunuz: tarama, parçalama, benzerlik araması, yeniden sıralama. Her biri ayrı bir kaybetme noktası.</span></li> <li className="flex items-start gap-3"><span className="text-violet-600 font-bold shrink-0 mt-0.5">✦</span><span>Cevabın kaynağı model değil, o an getirilen belge. Bu yüzden içeriğin tazeliği eğitim verisinden daha çok işe yarıyor.</span></li> <li className="flex items-start gap-3"><span className="text-violet-600 font-bold shrink-0 mt-0.5">✦</span><span>Sistem, kaynak bilgisini takip edemediğinde atıf da veremiyor. Atıf almak istiyorsanız içeriğinizin kimden geldiği makine tarafında izlenebilir olmalı.</span></li> <li className="flex items-start gap-3"><span className="text-violet-600 font-bold shrink-0 mt-0.5">✦</span><span>Türkçe sorgularda getirilen kaynak havuzu İngilizce'ye göre çok daha dar. Bu bir dezavantaj değil, henüz doldurulmamış bir alan.</span></li></ul>
                </aside>
  ...
  <table>
    ...
    <thead>
      <tr><th scope="col">Boyut</th><th scope="col">Grounding</th><th scope="col">RAG</th></tr>
    </thead>
    <tbody>
      <tr>
        <th scope="row">Ne olduğu</th>
        <td>Çıktıyı doğrulanmış harici bilgiye dayandırma pratiği — bir tasarım hedefi</td>
        <td>Belge getirip üretime ekleme yöntemi — hedefe giden bir yol</td>
      </tr>
      <tr>
        <th scope="row">Kapsam</th>
        <td>RAG'ı içerir, aşar</td>
        <td>Grounding'in alt kümesi</td>
      </tr>
      <tr>
        <th scope="row">Yöntemler</th>
        <td>Araç çağrısı (CRM, API), yapılandırılmış veri sorgusu, bilgi grafiği gezintisi, RAG</td>
        <td>Parçalama, embedding, vektör araması, yeniden sıralama, üretim</td>
      </tr>
      <tr>
        <th scope="row">Nerede tökezler</th>
        <td>Çok kaynaklı kurgularda kaynaklar çeliştiğinde orkestrasyon gerekir</td>
        <td>Cevap bir metin belgesinde değil işlemsel kayıtta yaşıyorsa bulamaz</td>
      </tr>
    </tbody>
  </table>
  {/* kaynak: K73 */}
  ...
  <p>
    RAG yeni bilgiyi modele çalışma anında, istemin içinde sunar; fine-tuning ise
    modelin ağırlıklarını yeni veriyle yeniden eğitir. Bilgi sık değişiyorsa RAG
    genelde daha pratik ve daha ucuz bir yol, çünkü modeli yeniden eğitmeden veri
    eklenebiliyor. Fine-tuning ise davranış ve üslup değiştirmek istediğinizde
    devreye giriyor. İkisi rakip değil, farklı işlerin araçları.
    {/* kaynak: K67 */}
  </p>

  <h3>Grounding halüsinasyonu tamamen önler mi?</h3>
  <p>
    Hayır. Modele istem içinde gerçek belgeler vermek uydurma oranını düşürür, ama
    iyi temellendirilmiş modeller bile aldıkları içeriği yanlış aktarabilir.
    {/* kaynak: K66, K73 */}
  </p>

  <h3>İçeriğim modelin eğitim verisinde yoksa hiç şansım yok mu?</h3>
  <p>
    Tam tersi. RAG'ın varlık sebebi bu: cevap eğitim verisinden değil, o an getirilen
    belgelerden kuruluyor. Dün yayımladığınız bir sayfa, bugün sorulan bir soruya
    kaynak olabilir. Bu yüzden tazelik ve taranabilirlik, modelin sizi tanıyor
    olmasından daha belirleyici.
  </p>

  <h3>Neden içeriğim getiriliyor ama atıf almıyorum?</h3>
  <p>
    İki ayrı ihtimal var. Ya yeniden sıralama adımında elendiniz — getirildiniz ama
    daha alakalı bulunan başka pasajlar üste çıktı. Ya da kaynak bilgisi net
    izlenemediği için sistem içeriği kullandı ama size referans veremedi. Birincisinin
    çaresi ayrışma, ikincisinin çaresi yazar, tarih ve kurum bağını makine tarafında
    net kurmak.
  </p>

  <h3>Tek adımlı RAG mı, agentic retrieval mı?</h3>
  <p>
    Basit arama görevlerinin çoğu için tek adımlı RAG yeterli. Çok adımlı akıl
    yürütme gerektiren, birden çok kaynağı birleştirmesi gereken sorularda agentic
    retrieval devreye giriyor.
    {/* kaynak: K73 */}
    İçerik tarafında bunun anlamı, karşılaştırma ve karar sorularının tek bir sayfayla
    değil, birbirine bağlı bir sayfa kümesiyle kazanıldığı. Alt sorular paralel
    yürütüldüğü için, o kümenin her parçasında ayrı ayrı görünür olmanız gerekiyor.
  </p>

  <h3>Verilerimin gizliliği RAG'da nasıl korunur?</h3>
  <p>
    Tamamen mimariye ve araç seçimine bağlı. Kendi altyapınızda çalışan bir arama
    katmanı kurduğunuzda verinin dışarı çıkmaması mümkün — Elasticsearch üzerine
    kurulan RAG için üretici bunu açıkça belirtiyor; satıcı beyanı olduğunu not
    edelim.
    {/* kaynak: K67 */}
    Harici bir API'ye bağlanan kurulumlarda ise sözleşme ve veri işleme koşulları
    belirleyici oluyor.
  </p>
