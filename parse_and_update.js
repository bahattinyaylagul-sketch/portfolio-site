...
<div className="max-w-7xl mx-auto px-4 md:px-6 relative flex flex-col lg:flex-row gap-12 items-start mb-24 mt-8">
    ...
    <main className="flex-1 w-full min-w-0 flex flex-col gap-y-24">
`;
content = content.replace(heroRegex, newHero);


// CLOSE THE MAIN TAG at the end of content
const ctaRegex = /(<section id="contact" className="mt-20">)/;
content = content.replace(ctaRegex, '</main>\n$1\n</div>\n');


// 2. REWRITE Success Stories into 4 large cards

function genCard(title, subtitle, perc1, p1T, perc2, p2T, perc3, p3T, logo, logoW, logoH, bgImg, link, scaleText) {
    return \`
        <div className="border border-gray-100 rounded-[2.5rem] overflow-hidden group hover:shadow-2xl hover:shadow-gray-200/50 transition-all duration-500 flex flex-col md:flex-row bg-white h-auto md:h-[420px]">
            <div className="p-8 md:p-12 flex flex-col justify-center relative z-10 w-full md:w-1/2">
                <div className="mb-6 h-14 flex items-center">
                    \${logo}
                </div>
                <h4 className="\${scaleText ? 'text-2xl md:text-3xl' : 'text-xl md:text-2xl'} font-black text-gray-900 mb-3">\${title}</h4>
                <p className="text-sm font-medium text-gray-500 mb-8 leading-relaxed">
                    \${subtitle}
                </p>
                <div className="grid grid-cols-3 gap-2 md:gap-4 mb-8">
                    <div className="text-left">
                        <div className="text-3xl md:text-4xl font-black text-[#58cc82] mb-1">\${perc1}</div>
                        <div className="text-xs md:text-sm font-bold text-gray-800 leading-tight">\${p1T}</div>
                    </div>
                    <div className="text-left">
                        <div className="text-3xl md:text-4xl font-black text-[#58cc82] mb-1">\${perc2}</div>
                        <div className="text-xs md:text-sm font-bold text-gray-800 leading-tight">\${p2T}</div>
                    </div>
                    <div className="text-left">
                        <div className="text-3xl md:text-4xl font-black text-[#58cc82] mb-1">\${perc3}</div>
                        <div className="text-xs md:text-sm font-bold text-gray-800 leading-tight">\${p3T}</div>
                    </div>
                </div>
            ...
            {/* Graphic area */}
            <div className="w-full md:w-1/2 relative h-64 md:h-full overflow-hidden pointer-events-none bg-gray-50 flex items-center justify-center">
...
const nKolayLogo = \`<Image src="/images/references/n-kolay-yeni.png" alt="N Kolay" width={160} height={50} className="object-contain" />\`;
const atlasLogo = \`<Image src="/images/references/atlas-uni.jpg" alt="Atlas Üniversitesi" width={160} height={50} className="object-contain" />\`;
const maltepeLogo = \`<Image src="/images/references/maltepe-uni.png" alt="Maltepe Hastanesi" width={160} height={50} className="object-contain" />\`;
const golfLogo = \`<Image src="/images/references/golf-yeni.png" alt="Golf Dondurma" width={120} height={50} className="object-contain" />\`;
...
const casesBlock = `
    <div className="space-y-12">
        \${genCard("Finans Sektöründe Radikal Büyüme", "Aylık 2M+ organik trafik ve Türkiye'nin en rekabetçi finans kelimelerinde eşsiz büyüme grafiği.", "%166", "Gösterim<br/>Artışı", "%133", "Trafik<br/>Artışı", "%52", "Konum<br/>İyileşmesi", nKolayLogo, 160, 50, "/images/technical_seo_cover_3d.png", "/vaka-calismalari/n-kolay", true)}
        
        \${genCard("Sağlık Sektöründe Radikal Büyüme", "Uyguladığımız Semantik SEO ve teknik iyileştirmeler ile organikte eşsiz büyüme grafiği.", "266K+", "Aylık<br/>Trafik", "39K+", "Sıralanan<br/>Kelime", "%98", "Index<br/>Oranı", atlasLogo, 160, 50, "/images/semantic_seo_3d.png", "/vaka-calismalari/atlas-universitesi", true)}

        \${genCard("Hatalı Migration Sonrası %100 Kurtarma", "Sert trafik düşüşünün ardından uygulanan kapsamlı teknik check-up ve reorganizasyon ile zirveye dönüş.", "370K+", "Zirve<br/>Trafik", "74K+", "Sıralanan<br/>Kelime", "%120", "Görünürlük<br/>Hacmi", maltepeLogo, 160, 50, "/images/crawling_indexing_3d.png", "/vaka-calismalari/maltepe-hastanesi", true)}

        \${genCard("FMCG Sektöründe Organik Liderlik", "Yüksek rekabetli alanda uygulanan &quot;Topic Authority&quot; inşası ve eşsiz performans artışı.", "45K+", "Aylık<br/>Trafik", "7K+", "Kelime<br/>Kazanımı", "#1", "Target<br/>Keyword", golfLogo, 120, 50, "/images/topic_authority_3d.png", "/vaka-calismalari/golf-dondurma", true)}

        \${genCard("0&apos;dan Zirveye Doktor & Hekim SEO&apos;su", "Uzmanlık, deneyim ve güvenilirlik (E-E-A-T) konseptlerinin kurgusuyla gelen başarı.", "25K+", "Aylık<br/>Trafik", "8K+", "Sıralanan<br/>Kelime", "#1", "Klinik<br/>Arama", mertLogo, 48, 48, "/images/eeat_trust_3d.png", "/vaka-calismalari/mert-dumantepe", true)}
    </div>
`;


const searchRegex = /<div className="max-w-5xl mx-auto space-y-8">[\s\S]*?(?=<\/section>\s*\{\/\* Testimonial)/;
